...
async function submitGeoJsonToDb(filePath?: string, verbose = true) {
	try {
		...
		const condensedFilePath = path.join(
			process.cwd(),
			'src/routes/api/submitGeoJson/lastGeoJsonErrors-condensed.json'
		);
		try {
		...
		return {
			...
			message:
				issues.length > 0 || Object.keys(errorMap).length > 0
...
function logError(
	errorMap: Record<string, { count: number; examples: any[] }>,
	type: string,
	attribute: string,
	value: any,
	error: string
