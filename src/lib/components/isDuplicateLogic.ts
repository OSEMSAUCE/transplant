...
export function getDuplicatedMask(values: (string | number | null)[]): boolean[] {
	...
	return values.map(val => 
		(val !== null && val !== '') ? (valueCount.get(val) ?? 0) > 1 : false
	);
}
 ...
 */
function createSignature(duplicateMask: boolean[], maxRows: number = 10): string {
...
export function getDuplicatePatternMask(values: (string | number | null)[]): DuplicatePattern[] {
	...
	const patternType = patternSignatures.length === 2 ? 'cropDuplicatePattern' : 'landDuplicatePattern';
